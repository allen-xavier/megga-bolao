...
function getNextDrawLabel() {
  ...
  for (let add = 0; add < 7; add += 1) {
    const candidate = new Date(
      nowSp.getTime() + add * 24 * 60 * 60 * 1000
    );
    ...
    return (
      <section className="rounded-3xl bg-[#0f1117] p-6 text-sm text-white/75 shadow-lg ring-1 ring-white/5">
        ...
        <p className="mt-4 text-sm text-white/65">
          Nenhum bolão ativo no momento. Volte mais tarde para acompanhar novas
          oportunidades.
        </p>
      </section>
    );
  ...
  return (
    <section className="space-y-4">
      {ativos.map((bolao) => {
        ...
        const participationLabel = inferredParticipant
          ? "Participando"
          : statusLabel;
              ...
              <header className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-[200px]">
                  ...
                  <p className="mt-1 text-xs text-white/65">
                    Bolão inicia em {formatStartsAt(startsAt)}
                  </p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-[#141823] px-4 py-3 text-right">
