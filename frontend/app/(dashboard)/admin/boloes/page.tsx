...
function AdminBoloesPageContent() {
  ...
  const renderList = (title: string, list: Bolao[]) => (
    <section className="space-y-4">
      ...
      {list.length === 0 ? (
        <p className="rounded-2xl bg-[#111218] p-4 text-sm text-white/70 ring-1 ring-white/5">Nenhum bolão encontrado.</p>
      ) : (
        list.map((bolao) => (
          <article key={bolao.id} className="rounded-3xl bg-[#111218] p-5 text-white shadow-lg ring-1 ring-white/5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-white/60">
                  Início: {formatSaoPaulo(bolao.startsAt)} - Cota: {formatCurrency(bolao.ticketPrice)} - Mínimo: {bolao.minimumQuotas} cotas
                </p>
                {(bolao.isParticipant ||
            ...
            <div className="mt-4 grid gap-3 text-sm text-white/70 md:grid-cols-3">
              <div className="rounded-2xl bg-white/5 px-4 py-3">
                ...
                <p className="mt-2 text-base font-semibold text-[#f7b500]">{formatCurrency(bolao.guaranteedPrize)}</p>
              </div>
              <div className="rounded-2xl bg-white/5 px-4 py-3">
